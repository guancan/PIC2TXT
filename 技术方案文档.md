...
│   └── xhs_note_service.py # 小红书笔记处理 负责笔记级别的操作，如解析笔记数据、提取图片链接、关联任务等
│
├── utils/                  # 工具模块
...
- **批量任务创建**：根据表格中的image_list字段创建OCR任务
- **结果回写**：将OCR处理结果回写到表格新字段image_txt中

### 10. 小红书笔记处理服务 (services/xhs_note_service.py)
- **笔记数据解析**：解析CSV中的小红书笔记数据结构
- **图片链接提取**：从image_list字段提取所有图片URL
- **任务关联管理**：维护小红书笔记与OCR任务之间的关系
- **结果汇总**：将同一笔记的多张图片OCR结果汇总为单一文本
...
- **CSV导入处理流程**：CSV上传 → 解析表格 → 提取图片链接 → 创建OCR任务 → 处理任务 → 结果汇总 → 回写表格 → 导出结果
